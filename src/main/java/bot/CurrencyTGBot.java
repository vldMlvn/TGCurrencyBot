        ...
        if (update.hasCallbackQuery()) {
            ...
            switch (callbackQuery) {
                ...
                case BACK_TO_MENU_CALLBACK -> {
                    showMainMenu(update.getCallbackQuery().getMessage().getChat());
                    convertAmount=0;
                }
                case "USD", "EUR" -> {
                ...
                case "uah-to-eur" -> getConvertRateToCurrency
                        (update.getCallbackQuery().getMessage().getChat(), Currency.EUR, convertAmount);
                case "usd-to-uah" -> getConvertRateToUAH
                        (update.getCallbackQuery().getMessage().getChat(), Currency.USD, convertAmount);
                case "eur-to-uah" -> getConvertRateToUAH
                        (update.getCallbackQuery().getMessage().getChat(), Currency.EUR, convertAmount);
            }
